...
class CustomHelper extends HtmlHelper
{
    ...
    public function getActiveClass($controller = array(), $action = 'index', $pass = null)
    {
        $tmpController = array();
        $tmpAction = array();
        if (!is_array($controller)) {
            $tmpController = array($controller);
        } else {
            $tmpController = $controller;
        }
        if (!is_array($action)) {
            $tmpAction = array($action);
        } else {
            $tmpAction = $action;
        }

        if (isset($this->request->params['controller']) && in_array($this->request->params['controller'], $tmpController)) {
            if (in_array($this->action, $tmpAction) && (isset($this->request->params['pass'][0]) && ($this->request->params['pass'][0] == $pass))) {
                return 'active';
            }
