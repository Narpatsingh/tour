...
class CommonComponent extends Component {
	...
	function loginWithUserId($userId, $cModel = false)
    {
        $loginModel = $cModel ? $cModel : 'User';
        $user = ClassRegistry::init($loginModel)->find('first', array('conditions' => array('id' => $userId), 'recursive' => -1));
        if (!empty($user)) {
            $this->Auth->login($user[$loginModel]);
        } else {
            return array('status' => 'fail', 'message' => 'Invalid UserId');
        }
    }
