<?php
App::uses('Vendor', 'mailProvider.php');

class syncMailsShell extends AppShell
{
	public $uses = array('User');
    public $currentDate = '';
    public $attachmentSavePath = '';
	public $fromAddress = array();
    function __construct()
    {
        $this->currentDate = date('Y-m-d');
		$this->attachmentSavePath = WWW_ROOT;
		$this->fromAddress = array(
			'user@mgdsw.info',
		);
    }
	public function main()
    {
        ...
        foreach ($userDetails as $key => $user) {
            $mail = new iMapMail('{' . $user['config'] . '}INBOX', $user['email'], $user['password']);
            $mail->connect();
            $options = array(
    ...
    {
		$users = $this->User->find("all",array('fields'=>array('id, email_config, email_address, email_password, last_sync_date, last_mail_id')));
        echo "<pre>";
		print_r($users);
		exit;
